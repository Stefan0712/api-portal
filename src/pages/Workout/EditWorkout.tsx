...
const EditWorkout: React.FC = () => {
    ...
    const populateData = () =>{
        if(workoutData){
            ...
            const tempPhases = workoutData.phases.map(phase=>({...phase, exercises: phase.exercises.map(exercise=>({...exercise, tempId: uuidv4()}))}));
            setPhases(tempPhases);
        }
    }
    const handleSaveWorkout = async (data: Workout) => {
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/workout/${id}`, data, {withCredentials: true});
