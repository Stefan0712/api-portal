...
const EditExercise: React.FC = () => {
    ...
    const [rest, setRest] = useState<number>(45);
    ...
    const handleSaveExercise = async (data: Exercise) =>{
        try{
            const response = await axios.put(`${process.env.REACT_APP_API_URL}/exercise/${id}`, data, {withCredentials: true});
            if(response.status === 200){
                navigate(`/exercises`);
                showMessage("Exercise updated successfully", "success");
            }else{
                showMessage("There was an error saving this exercise", "error");
            }
        } catch (error){
