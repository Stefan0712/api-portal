...
const EditEquipment: React.FC<Props> = ({equipment, closeEditEquipment}) => {
    ...
    const [showGroups, setShowGroups] = useState(false);
     ...
     const [urlName, setUrlName] = useState(equipment.urlName || '');
     const [attributes, setAttributes] = useState<EquipmentAttributes[]>(equipment.attributes || []);
     const [tags, setTags] = useState<Tag[]>(equipment.tags || []);
     const [muscleGroups, setMuscleGroups] = useState<ITargetGroups[]>(equipment.muscleGroups || []);
    ...
    const handleSaveEquipment = () =>{
        ...
        if(userId && name && tags && name.length > 3 && name.length < 15 && tags.length > 0){
    ...
    return ( 
        <div className="absolute bottom-[25px] left-1/2 transform -translate-x-1/2 w-1/2 min-w-[400px] max-w-[800px] p-[20px] rounded grid grid-cols-[1fr_1fr] grid-rows-[50px_1fr_50px] gap-[10px] primary-color">
            <div className="col-start-1 col-end-3 row-start-1 row-end-2 flex items-center"><h2 className="font-bold text-xl">Create Equipment</h2><button className="ml-auto" onClick={closeEditEquipment}><img src={IconLibrary.Close} className="h-[40px] w-[40px]" /></button></div>
            <div className="space-y-4 col-start-1 col-end-2 row-start-2 row-end-3">
                <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="text" id="name" name="name" onChange={(e) => setName(e.target.value)} value={name} placeholder="Name" />
                <textarea className="h-[100px] w-full rounded secondary-color pl-[10px] pt-[10px]" id="description" name="description" onChange={(e) => setDescription(e.target.value)} value={description} placeholder="Description" />
                <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="text" id="urlName" name="urlName" onChange={(e) => setUrlName(e.target.value)} value={urlName} placeholder="URL Name" />
                <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="url" id="url" name="url" onChange={(e) => setUrl(e.target.value)} value={url} placeholder="URL" />
                <div className="flex flex-col gap-4">
                    <h3 className="font-bold">Attributes</h3>
                    <div className="w-full gap-3 grid grid-cols-[3fr_1fr_1fr_50px]">
                        <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="text" id="attrName" name="attrName" onChange={(e) => setAttributeName(e.target.value)} value={attributeName} placeholder="Name" />
                        <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="text" id="attrUnit" name="attrUnit" onChange={(e) => setAttributeUnit(e.target.value)} value={attributeUnit} placeholder="Unit" />
                        <input className="h-[40px] w-full rounded secondary-color pl-[10px]" type="text" id="attrValue" name="attrValue" onChange={(e) => setAttributeValue(e.target.value)} value={attributeValue} placeholder="Value" />
                    ...
                    <div className="secondary-color h-[200px] flex flex-col gap-2 rounded">
                        {attributes?.map((attribute, index) => (
                            <div key={index} className="flex w-full secondary-color rounded h-[40px] items-center px-[15px] gap-4"><h3>{attribute.name}</h3><p className="ml-auto">{attribute.value} {attribute.unit}</p><button onClick={()=>setAttributes(prev=>[...prev.filter(item=>item != attribute)])}><img className="w-[30px] h-[30px]" alt="" src={IconLibrary.Close} /></button></div>
                        ))}
                    </div>
                </div>
            </div>
            <div className="space-y-4 flex flex-col h-full col-start-2 col-end-3 row-start-2 row-end-3">
                ...
                <div className="flex gap-3 flex-col secondary-color h-[200px] gap-2 rounded">
                    {tags?.map((item, index)=>(<div key={"tag-"+index} className="h-[40px] px-[10px] rounded secondary-color flex items-center gap-3">
                ...
                <div className="flex flex-col gap-2 h-[200px] overflow-x-hidden overflow-y-auto secondary-color rounded p-2 pr-[20px]">
